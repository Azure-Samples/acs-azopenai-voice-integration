...
class CallHandler:
    ...
    async def handle_play(
        self, call_connection_id: str, text_to_play: str, context: str
    ) -> Optional[dict]:
        """
        ...
        try:
            ...
            play_source = TextSource(
                text=text_to_play, voice_name=self.config.VOICE_NAME
            )

            connection = self.call_automation_client.get_call_connection(
                call_connection_id
            )
            await connection.play_media_to_all(play_source, operation_context=context)
            return {"data": "200 OK"}
